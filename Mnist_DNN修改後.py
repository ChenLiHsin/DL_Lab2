...
# BATCH_SIZE = 64  # 原本的BATCH_SIZE
BATCH_SIZE = 400  # 修改後的BATCH_SIZE
sum = 0
# steps = 2000  # 原本的step
steps = 20000 # 修改後的step
...
# W_1 = tf.Variable(tf.random_uniform([784, 100])) # 784x100 weight matrix. 原始程式
W_1 = tf.Variable(tf.random_uniform([784, 250]))  # 將100修改為250
# b_1 = tf.Variable(tf.zeros([100])) # 100-element bias vector. 原始程式
b_1 = tf.Variable(tf.zeros([250]))  # 將100改為250
layer_1 = tf.nn.relu(tf.matmul(x, W_1) + b_1) # Output of hidden layer.

# W_2 = tf.Variable(tf.random_uniform([100, 10])) # 100x10 weight matrix. 原始程式
W_2 = tf.Variable(tf.random_uniform([250, 10]))  # 將100修改為250
