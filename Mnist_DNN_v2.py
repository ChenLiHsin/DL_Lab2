...
x = tf.placeholder(tf.float32, [None, 784], name="input") # Placeholder for input.
y = tf.placeholder(tf.float32, [None, 10], name="label") # Placeholder for labels.
...
pred = tf.argmax(layer_2, 1, name='pred')
...
with tf.Session() as sess: # Connect to the TF runtime.
  ...
  saver.save(sess, model_path+'/mnist-dnn-model.ckpt')
